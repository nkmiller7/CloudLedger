...
const categories = {
  ...
  create_category: async (creator_id, name) => {
    //console.log("[DAL create_category] creator_id:", creator_id, "name:", name);
    try {
      name = name.trim();
      creator_id = creator_id.trim();
      validate.categories.name(name);
      ...
      console.error("[DAL create_category] Error:", err);
      throw err;
    }
  },
  ...
  create_expense: async (
    ...
    validate.misc.object_id(category_id);
    validate.categories.expense.amount(amount);
    validate.categories.expense.description(description);
    validate.categories.expense.transaction_date(transaction_date);
    validate.categories.expense.payment_method(payment_method);
