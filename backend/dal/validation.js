  ...
  routes: {
    users: {
      ...
      update_settings: (body) => {
        ...
        validate.users.monthly_budget(body.monthly_budget);
      },
    },
    ...
    saving_goals: {
      ...
      add_contribution: (body) => {
        ...
        validate.saving_goals.contribution_amount(body.amount);
      },
    },
  },
  ...
  users: {
    monthly_budget: (monthly_budget) => {
      if (typeof monthly_budget !== "number")
        throw {
          http_code: 400,
          reason: "monthly_budget must be a number",
          trace: new Error().stack,
        };
      if (monthly_budget <= 0)
        throw {
          http_code: 400,
          reason: "monthly_budget must be greater than 0",
          trace: new Error().stack,
        };
      const budget_str = monthly_budget.toFixed(2);
      if (!/^[0-9]+\.[0-9]{2}$/.test(budget_str))
        throw {
          http_code: 400,
          reason:
            "monthly_budget must be a decimal number with exactly two decimal places",
          trace: new Error().stack,
        };
    },
    username: (username) => {
  ...
  categories: {
    ...
    expense: {
      amount: (amount) => {
        ...
        if (!/^[0-9]+\.[0-9]{2}$/.test(amount_str))
          throw {
            http_code: 400,
            reason:
              "amount must be a decimal number with exactly two decimal places",
            trace: new Error().stack,
          };
      },
      ...
      transaction_date: (transaction_date) => {
        ...
        if (isNaN(date.getTime()) || date.toISOString() !== transaction_date)
          throw {
            http_code: 400,
            reason:
              "transaction_date must be a valid date string in ISO 8601 format",
            trace: new Error().stack,
          };
      },
      ...
      frequency: (frequency) => {
        ...
        if (
          ...
          throw {
            http_code: 400,
            reason:
              "frequency must be one of 'one_time', 'daily', 'weekly', 'biweekly', 'monthly', or 'yearly'",
            trace: new Error().stack,
          };
      },
    },
    ...
    goal_amount: (goal_amount) => {
      ...
      if (!/^[0-9]+\.[0-9]{2}$/.test(goal_amount_str))
        throw {
          http_code: 400,
          reason:
            "goal_amount must be a decimal number with exactly two decimal places",
          trace: new Error().stack,
        };
    },
    current_amount: (current_amount) => {
      ...
      if (!/^[0-9]+\.[0-9]{2}$/.test(current_amount_str))
        throw {
          http_code: 400,
          reason:
            "current_amount must be a decimal number with exactly two decimal places",
          trace: new Error().stack,
        };
    },
